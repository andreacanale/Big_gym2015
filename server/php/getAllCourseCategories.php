...
if (mysqli_connect_errno()) { //verify connection
 ...
 //   echo "Successful connection"; // connection ok
    ...
    $query1 = "SELECT * FROM coursecategory;";
    $query2 = "SELECT content FROM paragraph WHERE id = 'P0004';";
    ...
    if($result1 and $result1->num_rows >0)
    {
        ...
        $myCourseCategories = array();//create an array
        while($row = $result1->fetch_array(MYSQL_ASSOC)) {
            array_push($myCourseCategories, $row);
          //  var_dump(json_encode($myArray));
        }
        $myResult["courseCategories"] = $myCourseCategories;
